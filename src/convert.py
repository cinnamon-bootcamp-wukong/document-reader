...
class FileConverter:
    ...
    def __convert_docx_to_pdf(docx_file_path, pdf_file_path):
        ...
        try:
            ...
            custom_css = CSS(filename=os.path.join(style_dir, 'custom.css'))
    ...
    def convert_to_image(self):
        """
        Converts a file to PNG and saves it.

        Args:
            file_path (str): The path to the file.

        Return: Output_folder_path (str): The path where the PNG files will be saved.
        """
        try:
            path = self.path
